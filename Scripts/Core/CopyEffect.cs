...
namespace Nexus.Core
{
    ...
    // }
    interface ICopyableEffect<T> where T : Effect, new()
    {
        public T Copy(T e);
    }
    ...
    class CopyEffect<T> where T : Effect, new()
    {
        readonly T Effect;
        public CopyEffect(T Effect)
        {
            this.Effect = Effect;
        }

        public static List<T> Verify(List<GameObject> objects)
        {
            List<T> count = new();
        ...
        bool Isnull()
        {
            MetricsManager.LogModError(XRL.ModManager.GetMod("vampirism"), "Warning in Alert.AddEffectToWitnesses<T> : CopyEffect<T> was provided a null instance of T Effect");
            return false;
        }

        bool Checksupport(T obj)
        {
            if (obj is null)
                MetricsManager.LogModError(XRL.ModManager.GetMod("vampirism"), "Warning in Alert.AddEffectToWitnesses<T> : CopyEffect<T> does not support the provided effect. Implement from ICopyableEffect<T> in your effect to provide support.");
            return obj is not null;
        }

        public bool TryCopy(out T obj) // recommended to use this one for safety!
        {
            obj = null;
        ...
        public T Copy() //supports many but not all basegame effects and mod effects that implement ICopyableEffect<T>
         =>
             Effect switch
             {
                 ICopyableEffect<T> e => Copy(e),
        ...
        public static T Copy(ICopyableEffect<T> e)
        {
            return e.Copy((T)e);
        }
