...
namespace Nexus.StealthSystem
{
    ...
    class SpotterGenerator
    {
        ...
        public SpotterGenerator(XRL.World.Parts.Stealth Source, List<GameObject> PotentialSpotters)
        {
            this.Source = Source;
        ...
        /// 
        bool Spotted(int distance, GameObject Spotter) => distance == Source.AI_RADIUS + 1 && Spotter.HasLOSTo(Source.ParentObject);
        string DefaultMessage(GameObject Spotter) => $"You try to sneak attack, but {Spotter.t()} spots you from a distance!";
        public static List<GameObject> GiveDefaultList(XRL.World.Parts.Stealth Source)
        {
            ...
            {
                if (!Scan.Unaware(witness, false) && !StealthCore.Inanimate(witness))
                    local.Add(witness);
            }
            return local;
        }
        ...
        {
            foreach (GameObject witness in PotentialSpotters)
        ...
        Spot SpotterFound<T>(GameObject Spotter, string message) where T : IOpinionSubject, new()
        {
            ...
            if (spot == Spot.SPOTTER_IN_DETECTION)
            {
                ...
                Spotter.ApplyEffect(new Spotter(Source.ParentObject, 5, true));
            }
            else
                Spotter.ApplyEffect(new Spotter(Source.ParentObject, 5, false));
            return spot;
        }
...
{
    /// <summary>
    /// Very simple pathing effect that removes itself when the player's feed is over. I removed the feeding interaction so, now its just an effect with a timer.
    /// </summary>
    ...
    public class Spotter : Effect
    {
        ...
        {
            if ((pathonly && Duration > 0) || (Duration > 0))
            {
