...
namespace XRL.World.Parts
{
	...
	public class Stealth : IPart //probably will be a skill eventually or an IScriptable
	{
		...
		void HaltStealthSystem(string text)
		{
			...
			ParentObject.SetStringProperty(Flag.STEALTH, bool.FalseString);
			StealthStage1 = false;
			StealthStage2 = false;
		}
		...
		//this should not be used for overall outcome, it is for skipping "resist attack" saves/checks 
		// (implying you are in a general stealth state - there is only one aware witness nearby, or none at all)
		//(as per current rules, lone witnesses cannot resist attacks, and people in LOS that are "unaware" (alseep, etc) cannot resist attacks)

		// you should use ValidateStealthATK() to determine the final result on whether or not the attack is "actually" a stealth attack
		//because it ascertains whether or not they are attacking the "correct witness" in case they are in a "one witness" situation at runtime
		//if they attack the wrong witness (return false), you should divert to a non-stealth attack
