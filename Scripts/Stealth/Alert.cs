...
namespace StealthSystem
{
    ...
    class Alert
    {
        readonly XRL.World.Parts.Stealth Source;
        ...
        public uint AoE(uint AoE) => AoE == default ? Source.AI_RADIUS : AoE;
        ...
        public Alert(XRL.World.Parts.Stealth Source, List<GameObject> Witnesses, GameObject Exposer = null)
        {
            this.Source = Source;
        ...
        bool Validated(GameObject obj, uint AoE) => obj is not null && obj.DistanceTo(Source.ParentObject) <= AoE;
        ...
        public static List<GameObject> GiveDefaultList(XRL.World.Parts.Stealth Source)
        {
            List<GameObject> local = new(); //Notice how we pull from ValidSentients, which has no LOS, awareness, or distance restrictions.
            foreach (GameObject obj in Source.ValidSentients)
            {
        ...
        public void RemoveSleepFromWitnesses(uint AoE = default)
        {
            AoE = this.AoE(AoE);
            List<GameObject> local = new(Witnesses);
            foreach (GameObject obj in local)
            {
                if (Validated(obj, AoE) && obj.HasEffect<Asleep>())
                {
                    obj.RemoveEffect<Asleep>();
                }
            }
        }
        ...
        {
            if (Target is not null && !Witnesses.Contains(Target))
            {
        ...
        {
            InternalAddEffect<T>(new CopyEffect<T>(obj), AoE);
        }
        ...
        void AddEffectToWitnessesAndExposer<T>(T obj, uint AoE = default) where T : Effect, new()
        {
            CopyEffect<T> copy = new(obj);
            if (copy.TryCopy(out T effect))
            {
                Exposer?.ApplyEffect(effect);
                InternalAddEffect<T>(copy, AoE);
            }
        }
        ...
        void InternalAddEffect<T>(CopyEffect<T> copy, uint AoE) where T : Effect, new()
        {
            AoE = this.AoE(AoE);
            List<GameObject> local = new(Witnesses);
            foreach (GameObject gameObject in local)
                if (Validated(gameObject, AoE) && copy.TryCopy(out T effect))
