...
namespace StealthSystem
{
    ...
    /// </summary>
    class ActiveStealth //i plan to one day, maybe, turn this into an "Actual" UI, so i made it a separate class to avoid serialization issues down the line post-release
    {
        readonly XRL.World.Parts.Stealth Source;
        const int SINGLE = 1;
        const int NONE = 0;
        readonly bool HideMessages;
        public ActiveStealth(XRL.World.Parts.Stealth Source, bool HideMessages)
        {
            this.Source = Source;
        ...
        void Single(int count)
        {
            if (!Source.StealthStage1)
            {
                ...
                Source.ParentObject.SetStringProperty(Flag.STEALTH, bool.TrueString);
            }
        }
        ...
        void Broken(int count)
        {
            if (Source.StealthStage1 || Source.StealthStage2)
            {
                ...
                Source.StealthStage2 = false;
                Source.StealthStage1 = false;
                Source.ParentObject.SetStringProperty(Flag.STEALTH, bool.FalseString);
            }
        }
